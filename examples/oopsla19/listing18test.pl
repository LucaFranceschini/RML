...
match(_, any).
trace_expression('Main', Main) :- DeqAfter=gen(['n', 'val'], guarded((var('n')>0), ((deq:eps)*app(DeqAfter, [(var('n')-1), var('val')])), ((deq(var(val)):eps)*1))), Queue=gen(['s'], ((var(val, ((enq(var(val)):eps)*((((deq>>app(DeqAfter, [var('s'), var('val')]));1))/\app(Queue, [(var('s')+1)]))))\/guarded((var('s')>0), ((deq:eps)*app(Queue, [(var('s')-1)])), eps))\/((size(var(s)):eps)*app(Queue, [var('s')])))), Main=(((relevant>>app(Queue, [0]));1)).
