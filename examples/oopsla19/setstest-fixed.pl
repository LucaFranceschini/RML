...
match(_, any).
trace_expression('Main', Main) :- Main=(((filter>>optional((star(no_rm:eps)*var(el, (acquire(var(el)):((Main|(star(use(var(el)):eps)*(release(var(el)):eps)))/\(((toCheck(var(el))>>(release(var(el)):1));1))))))));1)).
