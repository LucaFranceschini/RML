...
match(_, any).
trace_expression('Main', Main) :- Stack=gen(['s'], optional((star(size(var(s)))*var(val, (push(var(val)):(app(Stack, [(var('s')+1)])*(pop(var(val)):app(Stack, [var('s')])))))))), Main=(((filter>>app(Stack, [0]));1)).
