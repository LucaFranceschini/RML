...
match(_, any).
trace_expression('Main', Main) :- Main=clos((((filter>>var(val, ((enq(var(val)):eps)*((((is_val(var(val))>>(star((enq(var(val)):eps))*((deq(var(val)):eps)*star((plus((enq(var(val)):eps))*(deq(var(val)):eps))))));1))/\(((is_not_val(var(val))>>Main);1))))));1))).
