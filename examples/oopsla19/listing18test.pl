...
match(_, any).
trace_expression('Main', Main) :- DeqAfter=gen(['n', 'val'], guarded((var('n')>0), (deq:app(DeqAfter, [(var('n')-1), var('val')])), (deq(var(val)):1))), Queue=gen(['s'], ((var(val, (enq(var(val)):((((deq>>app(DeqAfter, [var('s'), var('val')]));1))/\app(Queue, [(var('s')+1)]))))\/guarded((var('s')>0), (deq:app(Queue, [(var('s')-1)])), eps))\/(size(var(s)):app(Queue, [var('s')])))), Main=(((filter>>app(Queue, [0]));1)).
