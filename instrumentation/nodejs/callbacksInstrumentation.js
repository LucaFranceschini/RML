...
const { sendEvent } = require('./send.js');
...
function traceCb(data) { // true if the callback needs to be traced
    return supportedNames === null || supportedNames.includes(data.functionName) || isInSupportedModule(data); 
}

function skip(data) { // optimization to monitor functions with names with initParams.names
    return !data.functionObject._jalangi_callId && supportedNames !== null && !supportedNames.includes(data.functionName);
}
