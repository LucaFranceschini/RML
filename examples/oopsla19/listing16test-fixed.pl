...
match(_, any).
trace_expression('Main', Main) :- Stack=gen(['id', 's'], (star(size(var(id), var(s)):eps)*optional(var(val, (push(var(id), var(val)):(app(Stack, [var('id'), (var('s')+1)])*(pop(var(id), var(val)):app(Stack, [var('id'), var('s')])))))))), Main=(((filter>>optional(var(id, (new(var(id)):(Main|(app(Stack, [var('id'), 0])*(free(var(id)):eps)))))));1)).
