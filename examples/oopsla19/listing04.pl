...
match(_, any).
trace_expression('Main', Main) :- Collection=(star(nonStruct)*(hasNext('false'):eps)), Iterator=((hasNext('false'):eps)\/(hasNext('true'):(next:Iterator))), Main=(star((Iterator|notIt))/\star((Collection|notCol))).
