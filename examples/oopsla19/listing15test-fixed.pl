...
match(_, any).
trace_expression('Main', Main) :- Stack=gen(['s'],(star(size(var(s)):eps)*optional(var(val, (push(var(val)):(app(Stack, [(var('s')+1)])*(pop(var(val)):app(Stack, [var('s')])))))))), Main=(((filter>>app(Stack, [0]));1)).
