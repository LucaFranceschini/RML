...
match(Event, notAcquire(Id)) :- not(match(Event, acquire(var(id)))).
match(_, any).
trace_expression('Main', Main) :- Main=(eps\/var(id, (acquire(var(id)):(star((((notAcquire(var(id)):1)/\Main)|use(var(id))))*(release(var(id)):eps))))).
