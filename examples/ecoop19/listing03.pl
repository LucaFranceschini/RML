...
trace_expression('NotIt', NotIt) :- NotIt=(eps\/(notIt:NotIt)), NotCol=(eps\/(notCol:NotCol)), NonStruct=(eps\/(nonStruct:NonStruct)), Collection=(NonStruct*(hasNext(var(false)):eps)), Iterator=(hasNext(var(false))\/(hasNext(var(true)):(next:Iterator))), Main=((Iterator|NotIt)/\(Collection|NotCol)).
