...
trace_expression('NotIt', NotIt) :- NotIt=(eps\/(notIt:NotIt)), NotCol=(eps\/(notCol:NotCol)), NonStruct=(eps\/(nonStruct:NonStruct)), Collection=(NonStruct*(hasNext('false'):eps)), Iterator=((hasNext('false'):eps)\/(hasNext('true'):(next:Iterator))), Main=((Iterator|NotIt)/\(Collection|NotCol)).
